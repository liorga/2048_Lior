...
public class DrawableTile extends Actor implements Observer {
    ...
    public void setBaseX() {
        switch (this.index % 4) {
            case 1:
                baseX = TILE_X + TILE_WIDTH + 15;
                break;
            case 2:
                baseX = TILE_X + 2 * (TILE_WIDTH + 15);
                break;
            case 3:
                baseX = TILE_X + 3 * (TILE_WIDTH + 15);
                break;
            default:
                baseX = TILE_X;
                break;
    ...
    public void setBaseY() {
        if (index >= 12) {
            baseY = TILE_Y - 3 * TILE_HEIGHT - 3 * 15;
        } else if (index >= 8) {
            baseY = TILE_Y - 2 * TILE_HEIGHT - 2 * 15;
        } else if (index >= 4) {
            baseY = TILE_Y - TILE_HEIGHT - 15;
        } else {
            baseY = TILE_Y;
        }
    }
